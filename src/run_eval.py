...
def make_responder(cfg: dict) -> Responder:
    ...
    Read the key from the environment, never from the config file:
        api_key = os.environ["MODEL_API_KEY"] if using nonlocal model
        
    Local Ollama needs no API key. Set OLLAMA_HOST only if you point at a
    remote/authenticated server; otherwise it defaults to localhost.
    
    Keep ret/timeout/retry logic here so the eval loop stays clean.

    THIS IS AN EXAMPLE RESPONDER ONLY. Do not attempt to red team against
    the model without appropriate authorizations. This may result in the 
    loss of priveleges or accounts on the associated platform.
    """
    host = os.environ.get("OLLAMA_HOST", "http://localhost:11434")
    model = cfg.get("model", "gemma3")
    timeout = cfg.get("timeout", 120)
    retries = cfg.get("retries", 3)
    options = cfg.get("options", {"temperature": 0})
    ...
    def responder(prompt: str) -> str:
        ...
        raise RuntimeError(
            "ollama chat failed after %d attempts: %s" % (retries, last_err)
        )
